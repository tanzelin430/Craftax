...
def main():
    ...
    print(f"🎯 Using {config.trainer.n_gpus_per_node} GPUs on {config.trainer.nnodes} node(s)")
...
class TaskRunner:
    def run(self, config):
        ...
        train_dataset = create_craftax_dataset(config.data.train_files, config.data, tokenizer, processor)
        val_dataset = None
